  ...
  return (
    <div className="bg-fondoCartelera w-full">
      ...
      <div className="cartelera grid grid-cols-2 md:grid-cols-5 grid-rows-5 md:grid-rows-2 gap-2 w-full mx-auto px-4 pb-2">
        {peliculas.map((pelicula, i) => ( // Mapeando sobre las peliculas recibidas
          <Card key={i} poster={pelicula.poster} titulo={pelicula.title}/> // Usando poster y titulo de cada pelicula
        ))}
      </div>
    </div>
  );
